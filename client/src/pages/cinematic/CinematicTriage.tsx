                  ...
                  <li key={message.id}>
                    ...
                    >
                      <span className="flex items-baseline justify-between gap-3">
                        <span className="truncate font-mono-tight text-[0.78125rem] text-[hsl(var(--brand-bone))]">
                          {message.displayName}
                        </span>
                      ...
                      <span className="line-clamp-2 font-mono-tight text-[0.75rem] leading-snug text-[hsl(var(--brand-bone-dim))]">
                        {message.subject}
                      </span>
                      {judged ? (
                        <span
                          className={`font-mono-tight text-[0.625rem] uppercase tracking-[0.2em] ${
                            judged.right
  ...
  return (
    <div className="min-w-0">
      <article className="rounded-2xl border border-[hsl(var(--brand-iron))] bg-[hsl(var(--brand-graphite)/0.5)] p-5">
        ...
        <dl className="mt-4 grid grid-cols-1 gap-x-4 gap-y-1 font-mono-tight text-[0.75rem] sm:grid-cols-[auto_minmax(0,1fr)]">
          <Row label="From">
        ...
        <div className="mt-4 flex flex-wrap gap-x-5 gap-y-1 font-mono-tight text-[0.6875rem] uppercase tracking-[0.16em]">
        ...
        <button
          ...
          className="mt-4 min-h-[36px] font-mono-tight text-[0.6875rem] uppercase tracking-[0.24em] text-[hsl(var(--brand-ash))] transition-colors hover:text-[hsl(var(--brand-bone))]"
        >
        ...
        {headersOpen ? (
          <div className="mt-2 overflow-x-auto rounded-xl border border-[hsl(var(--brand-iron))] bg-[hsl(var(--brand-obsidian)/0.7)] p-3">
            <pre className="font-mono-tight text-[0.71875rem] leading-relaxed text-[hsl(var(--brand-bone-dim))]">
        ...
        <div className="mt-5 border-t border-[hsl(var(--brand-iron))] pt-4">
          {message.body.map((paragraph, index) => (
            <p
              key={index}
              className="mt-3 font-mono-tight text-[0.84375rem] leading-[1.75] text-[hsl(var(--brand-bone-dim))] first:mt-0"
            >
              {paragraph}
            </p>
          ))}
        ...
        {message.links.length > 0 ? (
          <div className="mt-5">
            <h3 className="font-techno text-[0.625rem] uppercase tracking-[0.4em] text-[hsl(var(--brand-ash))]">
              · Links, with where they go
            </h3>
            <ul className="mt-2 space-y-1.5">
              {message.links.map((link, index) => (
                <li key={index} className="font-mono-tight text-[0.75rem] leading-snug">
          ...
          <div className="mt-5">
            ...
            <ul className="mt-2 space-y-1.5">
              {message.attachments.map((attachment, index) => (
                <li
                  key={index}
                  className="font-mono-tight text-[0.75rem] leading-snug text-[hsl(var(--brand-bone-dim))]"
                >
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button
          ...
          className={`min-h-[44px] rounded-lg border px-6 font-mono-tight text-[0.6875rem] uppercase tracking-[0.24em] transition-colors ${
            judged?.called === "phish"
        ...
        <>
          {judged.called === "phish" && !judged.cited ? (
            <section className="mt-4 rounded-2xl border border-[hsl(var(--brand-iron))] bg-[hsl(var(--brand-graphite)/0.4)] p-5">
              <h3 className="font-techno text-[0.625rem] uppercase tracking-[0.4em] text-[hsl(var(--brand-signal))]">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {ALL_TELLS.map((tell) => (
                  <button
                    ...
                    className="rounded-full border border-[hsl(var(--brand-iron))] px-3 py-1.5 font-mono-tight text-[0.6875rem] text-[hsl(var(--brand-bone-dim))] transition-colors hover:border-[hsl(var(--brand-signal)/0.6)] hover:text-[hsl(var(--brand-bone))]"
                  >
                    {TELL_LABEL[tell]}
                  </button>
                ))}
  ...
  */
  ...
  return (
    <section className={`mt-4 rounded-2xl border p-5 ${shell}`} data-testid="triage-verdict">
      <h3 className={`font-techno text-[0.625rem] uppercase tracking-[0.4em] ${heading}`}>
        ·{" "}
        {judged.right
      ...
      {judged.cited ? (
        <p className="mt-3 font-mono-tight text-[0.78125rem] leading-relaxed text-[hsl(var(--brand-bone-dim))]">
      ...
      {message.tells.length > 0 ? (
        <div className="mt-4">
          <h4 className="font-techno text-[0.625rem] uppercase tracking-[0.32em] text-[hsl(var(--brand-danger))]">
            · What settles it
          </h4>
          <ul className="mt-2 space-y-2">
            {message.tells.map((tell) => (
              <li key={tell} className="font-mono-tight text-[0.78125rem] leading-relaxed">
      ...
      {message.redHerrings.length > 0 ? (
        <div className="mt-4">
          <h4 className="font-techno text-[0.625rem] uppercase tracking-[0.32em] text-[hsl(var(--brand-amber))]">
            · Present, and proves nothing
          </h4>
          <ul className="mt-2 space-y-2">
            {message.redHerrings.map((herring) => (
              <li key={herring} className="font-mono-tight text-[0.78125rem] leading-relaxed">
