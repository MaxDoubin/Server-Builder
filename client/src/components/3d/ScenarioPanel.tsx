  ...
  return (
    <li className="rounded-md border border-white/10 bg-white/5 p-2">
      <div className="flex items-start gap-2">
        <span
          aria-hidden
          className={`mt-0.5 inline-flex h-4 w-4 shrink-0 items-center justify-center rounded-sm border text-[0.625rem] leading-none ${
            met
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-baseline justify-between gap-2">
            ...
            <span
              className={`shrink-0 rounded-sm px-1 text-[0.625rem] uppercase tracking-widest ${
          ...
          <p className="mt-0.5 text-[0.625rem] leading-relaxed text-white/45">{objective.detail}</p>
          ...
          <div className="mt-1 flex items-baseline justify-between gap-2 text-[0.625rem]">
...
export function ScenarioPanel({
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      <span className="rounded-sm bg-cyan-400/15 px-1 text-[0.625rem] uppercase tracking-widest text-cyan-200">
                        {DISCIPLINE_LABEL[item.discipline]}
                      </span>
                      {done && (
                        <span className="rounded-sm bg-emerald-400/15 px-1 text-[0.625rem] uppercase tracking-widest text-emerald-200">
                          Complete
                        </span>
                      )}
                    </div>
  ...
  return (
    <div className="space-y-2.5">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="rounded-sm bg-cyan-400/15 px-1 text-[0.625rem] uppercase tracking-widest text-cyan-200 w-max">
        ...
        <div className="shrink-0 text-right">
          <div className="font-mono text-[0.6875rem] text-white/70">{formatElapsed(elapsedSeconds)}</div>
          <div className="text-[0.625rem] uppercase tracking-widest text-white/50">
      ...
      {complete ? (
        <div className="space-y-2 rounded-lg border border-emerald-400/40 bg-emerald-400/10 p-2.5">
          <div className="text-[0.625rem] uppercase tracking-widest text-emerald-200">
            Debrief · closed in {formatElapsed(elapsedSeconds)}
          </div>
          <p className="text-[0.6875rem] leading-relaxed text-emerald-50/90">{scenario.debrief}</p>
        </div>
      ) : (
        <>
          <p className="text-[0.6875rem] leading-relaxed text-white/70">{scenario.briefing}</p>
          <p className="rounded-md border border-white/10 bg-white/5 px-2 py-1.5 text-[0.625rem] leading-relaxed text-white/55">
      ...
      {!complete && (
        <details className="rounded-md border border-white/10 bg-white/5 px-2 py-1.5">
          <summary className="cursor-pointer text-[0.625rem] uppercase tracking-widest text-white/50">
            Hints
          </summary>
          <ul className="mt-1.5 space-y-1">
            {scenario.hints.map((hint) => (
              <li key={hint} className="text-[0.625rem] leading-relaxed text-white/55">
                · {hint}
              </li>
            ))}
          </ul>
