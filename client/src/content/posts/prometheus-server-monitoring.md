...
`for: 5m` means the alert goes to the `pending` state on the first evaluation where the expression is true and only becomes `firing` if it has been true at every evaluation since. Not true again five minutes later: true at each one in between. A single evaluation where the expression returns nothing clears the clock rather than pausing it, so a metric that dips under the line once every four minutes goes pending on schedule and never fires at all. This suppresses the momentary spikes that make people mute a channel, and it is also the most common reason a correct-looking rule has no history. Detection time, when nothing dips, is `for` rounded up to a multiple of `evaluation_interval`, plus up to one more interval before the condition is first noticed. [The alert was pending all day](/blog/the-alert-was-pending-all-day) works through the failure modes, and [the graph crossed the line](/alerts) runs ten of them.
